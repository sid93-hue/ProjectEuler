...
from eulerlib.operations import does_number_have_zero, is_number_even, \
        is_digit_even, is_prime, num_digits, rotate_number, last_digit


def disqualify_conditions(n):
    """Function returns 1 if n cannot be a circular prime. If it returns 0
    n may or may not be a circular prime."""
...
def is_circular_prime(n):
    ...
    if disqualify_conditions(n):
        ...
        for i in range(0, operand_digits):
            logging.debug("Testing operand = %d ...", operand)
            if is_prime(operand):
                operand = rotate_number(operand)
            else:
                logging.debug("is_circular_prime : operand = %d is NOT Prime. E\
                        xiting function...", operand)
                return 0
        return 1
...
def main():
    ...
    print(f"ANSWER = {len(result_list) + 4}")  # Add 4 as this does account for
    # circular primes < 10
